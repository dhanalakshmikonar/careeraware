...
@section('title', 'My Career Report - WatyAssessment')
    ...
    <div class="col-lg-12">
        <div class="glass-card p-5">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
                <div>
                    <h1 class="fw-bold mb-1 text-glow-cyan" style="color: var(--accent-secondary);">My Career Report</h1>
                    @if($result)
                        <p class="text-muted mb-0">Generated on {{ $result->created_at->format('M d, Y') }}</p>
                    @else
                        <p class="text-muted mb-0">Complete the assessment to generate your report.</p>
                    @endif
                </div>
                @if($result)
                    <div>
                        <a href="{{ route('student.results.report') }}" class="btn btn-glass-secondary">
...
@if(!$result || empty($result->top_careers))
    <div class="glass-card p-5 text-center text-muted">
        ...
        <p class="mb-4">You haven't completed an assessment yet.</p>
        <a href="{{ route('student.assessment') }}" class="btn btn-premium">Take the Assessment</a>
    </div>
@else
    <div class="row g-4">
        ...
        <div class="col-lg-4">
            <div class="glass-card p-4">
                ...
                <p class="text-muted small mb-4">Your raw interest scores across all technical career paths.</p>

                <div class="d-flex flex-column gap-3">
                    @php
                        $scores = $result->career_scores ?? [];
                        arsort($scores);
                        $maxScore = count($scores) > 0 && max($scores) > 0 ? max($scores) : 1;
                    @endphp
                        ...
                        <div>
                            ...
                            <div class="progress bg-dark" style="height: 6px; border: 1px solid rgba(255, 255, 255, 0.05);">
                                <div class="progress-bar" role="progressbar"
                                    style="width: {{ ($score / $maxScore) * 100 }}%; background: linear-gradient(90deg, var(--accent-primary), var(--accent-secondary));"></div>
                            </div>
                        </div>
                    @endforeach
                </div>
